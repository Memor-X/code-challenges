...
function Get-Hand-Type($cardArr)
{
    ...
    $cardArrClean = $()
    $cardArrClean = foreach($cardInt in $cardArr)
    {
        if($cardInt -ne 1)
        {
            $cardInt
    ...
    $jokerCount = $cardArr.Length - $cardArrClean.Length

    $uniqCards = ($cardArrClean | Sort-Object | Get-Unique)
    $grouping = ($cardArrClean | Group-Object | ForEach-Object { $h = @{} } { $h[$_.Name] = $_.Count + $jokerCount } { $h })
