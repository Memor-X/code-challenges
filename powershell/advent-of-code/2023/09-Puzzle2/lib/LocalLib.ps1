...
function Predict-Dataset($dataset, $side = 'r')
{
    if($side -eq 'l')
    {
        Write-Log "Reversing array"
        for($i = 0; $i -lt $dataset.Length; $i += 1)
        {
            [array]::Reverse($dataset[$i])
        }
    }
    ...
    if($side -eq 'l')
    {
        Write-Log "Reverting Array Reversal"
        for($i = 0; $i -lt $dataset.Length; $i += 1)
        {
            [array]::Reverse($dataset[$i])
        }
